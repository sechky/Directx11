...
void Shader::CreateInputLayout()
{
	...
	//vertex가 가진 정보의 갯수를 구해서 각 정보를 기록하여 배열에 담는다.
	for (UINT i = 0; i< shaderDesc.InputParameters; i++)
	{
		D3D11_SIGNATURE_PARAMETER_DESC paramDesc;
		reflection->GetInputParameterDesc(i, &paramDesc);
		//원래 inputlayout이 어떻게 들어가는지 정해주고 보내줬지만
		//보내준 자료를 자동 처리하여 넣어주게 만듦

		D3D11_INPUT_ELEMENT_DESC elementDesc;
		elementDesc.SemanticName = paramDesc.SemanticName;
		elementDesc.SemanticIndex = paramDesc.SemanticIndex;
		elementDesc.InputSlot = 0;
		elementDesc.AlignedByteOffset = D3D11_APPEND_ALIGNED_ELEMENT;//자료형의 크기만큼 값을 넣어야 하지만 D3D11_APPEND_ALIGNED_ELEMENT으로 설정할 경우 이전 값의 위치 다음의 offset로 설정해준다. 
		...
		//D3D11_APPEND_ALIGNED_ELEMENT를 사용하여 하나의 원소씩 건너 띄므로 Position크기에 맞는 자료형으로 맞춰 줘야한다.
	...
	(
		&inputLayoutDesc[0]//정보가 들어있는 메모리 시작주소
		, inputLayoutDesc.size()//배열의 크기는 정보의 갯수이다
		, vertexBlob->GetBufferPointer()
		, vertexBlob->GetBufferSize()
		, &inputLayout
	);
